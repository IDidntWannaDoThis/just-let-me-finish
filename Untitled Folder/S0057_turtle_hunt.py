...
class max(turtle.Turtle):
    ...
    def rotate_prey(self, positions, hunters): # turtle will be turned right <degree> degrees. Use negative values for left turns.
        closehunter =hunters[0]
        for h in hunters:
            if distance(self.position(), h.position()) < distance(self.position(), closehunter.position()):
                closehunter = h
        ...
        degree = -self.heading() + -direction(self, closehunter)
        # print(positions[0][1])
        return degree

    def rotate_hunter(self, positions,the_pray):  # turtle will be turned right <degree> degrees. Use negative values for left turns.
        degree = self.heading() + direction(positions, the_pray)
        return degree
...
def hunt(prey_class, hunter_class, color):  # execute the hunt
    ...
    while not caught(turtles, CAUGHT_DISTANCE) and turn < MAX_TURNS:
        turn += 1
        for h in hunters:
            h.right(h.rotate_hunter(h, prey))
        prey.right(prey.rotate_prey(positions, hunters))
        for t in turtles:
            move(t)
            positions = [t.position() for t in turtles]  # this is list comprehension https://www.w3schools.com/python/python_lists_comprehension.asp
...
class1 = max  # (red prey) Replace PlayerName1 by your own class name here.
